...
function createCustomerAuth(store, { readJson }) {
  const empty = { users: [], sessions: [] };
  const siteOrigin = (req) => process.env.SITE_URL ? new URL(process.env.SITE_URL).origin : `${req.socket.encrypted ? "https" : "http"}://${req.headers.host}`;
  const requireOrigin = (req) => {
    if (req.headers.origin && req.headers.origin !== siteOrigin(req)) error("Origen de solicitud no permitido.", 403);
  };
  ...
  async function handle(req, res, url) {
    ...
    if (url.pathname === "/api/auth/session" && req.method === "GET") {
      ...
      res.end(JSON.stringify({ ok: true, authenticated: Boolean(user), user: user ? publicUser(user) : null }));
      return true;
    }
    ...
    if (url.pathname === "/api/auth/register") {
      ...
      res.end(JSON.stringify({ ok: true, user: publicUser(createdUser) }));
      return true;
    }
