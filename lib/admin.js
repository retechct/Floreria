...
const { seedCatalog, publicCatalog, normalizeProduct, normalizeCollection, normalizeCategory, fail } = require("./catalog");
...
function requireOrigin(req) {
  const expected = process.env.SITE_URL ? new URL(process.env.SITE_URL).origin : `${req.socket.encrypted ? "https" : "http"}://${req.headers.host}`;
  // Proxied deployments must declare their public SITE_URL explicitly.
...
function createAdmin(store, { reconcilePayment } = {}) {
  ...
  const getCatalog = () => store.read("catalog", seed);
  ...
  async function handle(req, res, url) {
    ...
    if (route === "/api/admin/login" && req.method === "POST") {
      ...
      if (!same(derived.toString("hex"), hash) || !same(String(body.username || ""), process.env.ADMIN_USERNAME || "admin")) fail("Usuario o contrasena incorrectos.", 401);
      ...
      json(res, 200, { ok: true, csrf: value.csrf });
      return true;
    }
